    ...
    def check_inst(self, inst, inst_group):
        if inst[0] == LEVEL.RA:
                ...
                if op1_device == op2_device:
                    ...
                    if op2_row_id > 0: # compute using gb
                        ...
                        for i, pu_id in enumerate(actual_pu_list):
                            ...
                            pu_sync_point = self.pu[pu_id].check_state() - SimConfig.burst_apox
            ...
            # op-level, op-type, ch_id, ra_id, device_mask(并口写入), (bank, row_id, col_offset), col_num, auto_precharge
                ...
                gb_sync_point = self.buffer.check_state() - SimConfig.burst_apox
        ...
        if inst[0] == LEVEL.RA:
            if inst[1] == OPTYPE.pu: 
                ...
                if op1_device == op2_device:
                    ...
                    pu_end = pu_start + (col_num) * max(SimConfig.tCCDL, SimConfig.burst_apox)
                    # bank read
                    for i, pu_id in enumerate(actual_pu_list):
            ...
            # NOTE: 先按照有转化来做
                ...
                buffer_end = buffer_start + (col_num - 1) * max(SimConfig.tCCDL, SimConfig.burst_apox) + SimConfig.ra_gb_wl
                # bank read
                for device_id in device_list:
                ...
                buffer_end = buffer_start + (col_num - 1) * max(SimConfig.tCCDL, SimConfig.burst_apox)
                # bank read
                for device_id in device_list:
                    for bank_id in bank_list:
                        self.devices[device_id].banks[bank_id]\
