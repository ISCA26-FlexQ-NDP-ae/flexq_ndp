...
class Device:
    ...
    def issue_inst(self, inst, inst_group):
        ...
        if inst[1] == OPTYPE.pu:
            ...
            src_last_write = src_first_write + (col_len - 1) * max(SimConfig.tCCDL, SimConfig.burst_apox)
            pu_first_read = sync_point - SimConfig.de_pu_bf_rl
            pu_last_read = pu_first_read + (col_len) * max(SimConfig.tCCDL, SimConfig.burst_apox) # TODO: check this logic
            for i, pu_id in enumerate(actual_pu_list):
            ...
            sync_point = self.last_cmd_info[inst_group]
            src_first_read = sync_point - SimConfig.RL
            src_last_read = src_first_read + (col_len - 1) * max(SimConfig.tCCDL, SimConfig.burst_apox)
            ...
            bank_last_read = bank_first_read + (col_len - 1) * max(SimConfig.tCCDL, SimConfig.burst_apox)
            ...
            gb_last_write = gb_first_write + (col_len) * max(SimConfig.tCCDL, SimConfig.burst_apox)
            ...
            bus_end = sync_point + (col_len) * max(SimConfig.tCCDL, SimConfig.burst_apox)
            ...
            bank_last_write = bank_first_write + (col_len - 1) * max(SimConfig.tCCDL, SimConfig.burst_apox)
            for bank_id in bank_id_list:
            ...
            gb_last_read = gb_first_read + (col_len) * max(SimConfig.tCCDL, SimConfig.burst_apox)
