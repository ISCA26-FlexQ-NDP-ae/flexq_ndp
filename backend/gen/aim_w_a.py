...
def aim_w_a(config, cmd_list, args,
    ...
    for i, cmd in tqdm.tqdm(enumerate(cmd_list), total=len(cmd_list), desc="Processing commands"):
        ...
        if consider_scale_buffer:
            ...
            require_new_scale = not A_scale_buffer.is_data_available(current_a_scale) or (scale_b_buf_col > 0 and not B_scale_buffer.is_data_available(current_b_scale))
        ...
        #         m_start_id = m_id
        # 结果记录
        if next_cmd is None or next_cmd.matrix_coord != current.matrix_coord or next_cmd.A_scale_group_b_id != current.A_scale_group_b_id:
            # print(f"compute A{current.A_pos}_s{current.A_scale_pos} B{current.B_pos}_s{current.B_scale_pos} Result{current.matrix_coord} k_scale_group_id({current.A_scale_group_b_id})")
            AB_col_rst.append(current.matrix_coord+(current.A_scale_group_b_id,))
        ...
        (not buffer_b and not current.same_B_row(next_cmd)) or \
            ...
            if not args.silent:
                print(f"compute A{current.A_pos}_s{current.A_scale_pos} B{current.B_pos}_s{current.B_scale_pos} Result{current.matrix_coord} k_scale_group_id({current.A_scale_group_b_id})", file=log_file)
        
        # Partial Buffer 维护 - 使用属性访问替代硬编码索引
        # assert next_cmd.A_scale_group_b_id == next_cmd.B_scale_group_b_id, f"A_scale_group_b_id, B_scale_group_b_id mismatch: {next_cmd.A_scale_group_b_id}, {next_cmd.B_scale_group_b_id}"
        new_added = None
        if next_cmd is None or next_cmd.matrix_coord != current.matrix_coord or next_cmd.A_scale_group_b_id != current.A_scale_group_b_id:
            # 先处理当前结果的汇入
            ...
            buffered_k_scale.append(current.A_scale_group_b_id)
            # 再预测下一次循环，会不会超过容量限制
        ...
        if new_added is not None and consider_result_buffer:
            ...
            if scale_b_buf_col > 0:
                compute_col = 2 * math.ceil(len(new_added[1]) * accumulation_precision / SimConfig.co_w)
