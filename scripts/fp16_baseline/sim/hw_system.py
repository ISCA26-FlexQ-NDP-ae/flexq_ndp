            ...
            if inst[1] == OPTYPE.host_read:
                ...
                for device_id in device_list:
                    ...
                    device_buffer_sync_point = self.channels[ch_id].ranks[ra_id]\
                        .devices[device_id].buffer.check_state() - SimConfig.burst_apox
                    # device inner bus / device outer bus / rank bus (same to device bus, ignore) / channel bus
                    device_inner_bus_sync_point = self.channels[ch_id].ranks[ra_id]\
                        .devices[device_id].bus.check_state()
                    device_outer_bus_sync_point = self.channels[ch_id].ranks[ra_id]\
                ...
                for device_id in device_list:
                    ...
                    for pu_id in pu_list:
                        pu_sync_point = max(pu_sync_point, self.channels[ch_id].ranks[ra_id]\
                            .devices[device_id].pu[pu_id].check_state() - SimConfig.burst_apox)
                    # device inner bus / device outer bus / rank bus (same to device bus, ignore) / channel bus
                    device_inner_bus_sync_point = self.channels[ch_id].ranks[ra_id]\
                        .devices[device_id].bus.check_state()
                    device_outer_bus_sync_point = self.channels[ch_id].ranks[ra_id]\
    ...
    def issue_inst(self, inst, inst_group):
        if inst[0] == LEVEL.SYS:
            if inst[1] == OPTYPE.host_read:
                ...
                bank_last_read = bank_first_read + (col_num - 1) * max(SimConfig.tCCDL, SimConfig.burst_apox)
                bus_start = sync_point
                bus_end = bus_start + (col_num - 1) * max(SimConfig.tCCDL, SimConfig.burst_apox) + SimConfig.burst_apox
                for device_id in device_list:
                ...
                bank_last_write = bank_first_write + (col_num - 1) * max(SimConfig.tCCDL, SimConfig.burst_apox)
                bus_start = sync_point
                bus_end = bus_start + (col_num - 1) * max(SimConfig.tCCDL, SimConfig.burst_apox) + SimConfig.burst_apox
                for device_id in device_list:
                    for bank_id in bank_list:
                ...
                bus_start = sync_point
                # bus_end = bus_start + (col_num - 1) * max(SimConfig.tCCDL, SimConfig.burst_apox) + SimConfig.burst_apox
                bus_end = bus_start + (col_num - 1) * SimConfig.burst_apox + SimConfig.burst_apox
                buffer_start = sync_point + SimConfig.burst_apox
                # buffer_end = buffer_start + (col_num - 1) * max(SimConfig.tCCDL, SimConfig.burst_apox) + SimConfig.de_gb_wl
                buffer_end = buffer_start + (col_num - 1) * SimConfig.burst_apox + SimConfig.de_gb_wl                
                # device buffer write
                for device_id in device_list:
                    self.channels[ch_id].ranks[ra_id]\
                ...
                bus_start = sync_point
                bus_end = bus_start + (col_num - 1) * SimConfig.burst_apox + SimConfig.burst_apox
                pu_start = sync_point + SimConfig.burst_apox
                pu_end = pu_start + (col_num - 1) * SimConfig.burst_apox + SimConfig.de_pu_bf_wl
                for device_id in device_list:
                    for pu_id in pu_list:
                        self.channels[ch_id].ranks[ra_id]\
                ...
                sync_point = self.last_cmd_info[inst_group]
                bus_start = sync_point
                bus_end = bus_start + SimConfig.burst_apox
                pu_start = sync_point
                pu_end = pu_start + SimConfig.burst_apox
                # issue
                for device_id in device_list:
                    for pu_id in pu_list:
                        self.channels[ch_id].ranks[ra_id]\
